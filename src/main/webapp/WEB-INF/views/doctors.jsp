        ...
        <div class="row">
            <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
                <div class="site-heading">
                    ...
                    <hr class="small">
                    <span class="subheading"><fmt:message key="doctorList"/></span>
                </div>
            </div>
        </div>
    </div>
...
<div class="container">
    <div class="col-lg-8 col-lg-offset-2 col-md-10 col-md-offset-1">
        <div class="post-preview">
            <h2 class="text-center"><fmt:message key="doctorList"/></h2>
            <hr>
            <form action="/userList">
                    <select name="sortingType">
                        ...
                        <option value="3"><fmt:message key="category"/></option>
                    </select>
                <input type="hidden" name="listType" value="doctorList">
                ...
                <c:forEach items="${doctors}" var="user">
                    <form action="/user">
                        <button type="submit" name="userId" value="${user.getId()}" class="btn-danger btn-link">
                            <strong><fmt:message key="userN"/>${user.id}:</strong> ${user.getName()} ${user.getSurname()}  |
                            <strong><fmt:message key="category"/>:</strong> ${user.getDoctorInfo().getCategory()}
                        </button>
                    </form>
                </c:forEach>
            ...
            <c:if test="${currentPage != 1}">
                <a href="/userList?listType=doctorList&currentPage=${currentPage - 1}&sortingType=${sortingType}"><<</a>
            </c:if>
            ...
            <c:if test="${pages != 1}">
                    <c:forEach begin="1" end="${pages}" var="i">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a href="/userList?listType=doctorList&currentPage=${i}&sortingType=${sortingType}">${i}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
            </c:if>
